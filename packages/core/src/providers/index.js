...
const initProviders = () => {
  return new Promise((resolve, reject) => {
    ...
    log('info', 'core/providers', 'Loading Providers')
    fs.readdir(path.resolve(__dirname, './protocolProviders'), (err, files) => {
      if (err) log('error', 'core/providers', err)
      var counter = files.length
      files.forEach(file => {
        import(`./protocolProviders/${file}`)
          .then((module) => {
            providers.push(module.default.providerRegistration)
            log('info', 'core/providers', `Loaded ${module.default.providerRegistration.id} (${module.default.providerRegistration.label})`)
            counter--
            if (counter === 0) { 
              loadDeviceProviders()
            }
          })
          .catch(err => log('error', 'core/providers', err))
        })
      })
            ...
            import(`./deviceProviders/${directory}`)
              .then((module) => {
                if (module.default.providerRegistration.protocol === 'Serial' && !isSerial) {
                  log('info', 'core/providers', `Skipping ${module.default.providerRegistration.id} (${module.default.providerRegistration.label}) as this system has no Serial Interfaces`)
                }
                else {
                  providers.push(module.default.providerRegistration)
                  log('info', 'core/providers', `Loaded ${module.default.providerRegistration.id} (${module.default.providerRegistration.label})`)
